...
def plot_from_monitor_results(monitor_dir, window=10):
    ...
    ax1.plot(range(n_episodes), pd.Series(data['episode_lengths']).rolling(window).mean())
    ax1.set_xlabel('episode')
    ax1.set_ylabel('episode length')
    ax1.grid('k--', alpha=0.6)

    ax2.plot(range(n_episodes), pd.Series(data['episode_rewards']).rolling(window).mean())
